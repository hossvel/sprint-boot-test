...
class SprintBootTestApplicationTests {
	...
	void testFindAll() {
		...
		when(icuentaRepository.findAll()).thenReturn(datos);

		// when
		List<Cuenta> cuentas = icuentaService.findAll();
		...
		verify(icuentaRepository).findAll();
	}
	...
	void testSave() {
		...
		// when
		Cuenta cuenta = icuentaService.save(cuentaPepe);
		// then
		assertEquals("Pepe", cuenta.getPersona());
		assertEquals(3, cuenta.getId());
		assertEquals("3000", cuenta.getSaldo().toPlainString());

		verify(icuentaRepository).save(any());
	}
}
